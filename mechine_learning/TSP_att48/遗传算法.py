    ...
    while len(children) < target_count:
        ...
        if male_index != female_index:
            if random.random() > crossover_rate:
                children.append(parents[male_index])
                children.append(parents[female_index])
            else:
...
#
# plt.plot(X, Y, 'b')
# plt.savefig('遗传算法最优路径.png', dpi=500, bbox_inches='tight')
# plt.show()

plt.plot(list(range(len(register))), register, 'b')
plt.savefig('遗传算法收敛过程.png', dpi=500, bbox_inches='tight')
plt.show()
